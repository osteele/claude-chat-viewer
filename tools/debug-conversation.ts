...
function showConversationsList(data: any[]) {
  console.log(`${colors.blue}Found ${colors.bold}${data.length}${colors.reset}${colors.blue} conversations:${colors.reset}\n`);

  data.forEach((conv, index) => {
    const messageCount = conv.chat_messages?.length || 0;
    const name = conv.name || "Unnamed";
    console.log(`  ${colors.gray}[${index}]${colors.reset} ${name} ${colors.gray}(${messageCount} messages)${colors.reset}`);
  });
...
function showConversation(conv: any, index: number) {
  console.log(`${colors.blue}Conversation [${index}]:${colors.reset} ${colors.bold}${conv.name || "Unnamed"}${colors.reset}\n`);
  ...
  conv.chat_messages?.forEach((msg: any, msgIndex: number) => {
    ...
    console.log(`  ${colors.gray}[${msgIndex}]${colors.reset} ${sender} ${colors.gray}(${contentCount} content items)${colors.reset}`);
    console.log(`      ${colors.gray}"${preview}${msg.text?.length > 60 ? "..." : ""}"${colors.reset}`);
  });
...
function showMessage(msg: any, convIndex: number, msgIndex: number) {
  ...
  console.log(`  ${colors.gray}${msg.text?.substring(0, 200)}${msg.text?.length > 200 ? "..." : ""}${colors.reset}`);
  ...
  msg.content?.forEach((item: any, itemIndex: number) => {
    ...
    const typeColor =
      ...
      type === "thinking" ? colors.magenta :
      type === "tool_use" ? colors.cyan :
      type === "tool_result" ? colors.yellow :
      colors.gray;

    console.log(`  ${colors.gray}[${itemIndex}]${colors.reset} ${typeColor}${type}${colors.reset}: ${colors.gray}${description}${colors.reset}`);
  });
...
function showContentItem(item: any, convIndex: number, msgIndex: number, contentIndex: number) {
  console.log(`${colors.blue}Content Item [${convIndex}][${msgIndex}][${contentIndex}]:${colors.reset}\n`);
...
function debugFile(filePath: string, convIndex?: number, msgIndex?: number, contentIndex?: number) {
  ...
  try {
    ...
    if (convIndex < 0 || convIndex >= conversations.length) {
      console.error(`${colors.red}Error:${colors.reset} Conversation index ${convIndex} out of range (0-${conversations.length - 1})`);
      process.exit(1);
    }
    ...
    if (!conv.chat_messages || msgIndex < 0 || msgIndex >= conv.chat_messages.length) {
      console.error(`${colors.red}Error:${colors.reset} Message index ${msgIndex} out of range (0-${(conv.chat_messages?.length || 0) - 1})`);
      process.exit(1);
    }
    ...
    if (!msg.content || contentIndex < 0 || contentIndex >= msg.content.length) {
      console.error(`${colors.red}Error:${colors.reset} Content index ${contentIndex} out of range (0-${(msg.content?.length || 0) - 1})`);
      process.exit(1);
    }
